...
describe("root route shell", () => {
  it("renders the nav with the three primary links", async () => {
    ...
    // findByRole (polling) for the first assertion to wait out that
    // initial load, then synchronous getByRole for the rest.
    expect(
      await screen.findByRole("link", { name: "kemist-dashboard" }),
    ).toBeInTheDocument();
